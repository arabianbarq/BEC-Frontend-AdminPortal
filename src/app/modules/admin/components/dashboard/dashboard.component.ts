...
export class DashboardComponent implements OnInit {
  ...
  weeklyTotalHours: any;
  annuallyTotalHours: any;
  jobTotal: any;
  ...
  getUserJobHoursList() {
    this._dashboardService.getUserJobHours().subscribe(response => {
      this.weeklyTotalHours = response.data.UserJobHoursResponse.WeeklyTotalHours;
      this.annuallyTotalHours = response.data.UserJobHoursResponse.AnnuallyTotalHours;
    });
  }
  ...
  jobsStatusCount(data) {
    ...
    this._jobListService.getJobsListGridData(data).subscribe(response => {
      ...
      this.jobTotal = response.data.jobsMainResponse;
    ...
    this.jobCount = this.jobTotal == undefined ? 0 : this.jobTotal;
  }
  ...
  onSubmit() {
    if (this.widgetForm.valid) {
      ...
      this._dashboardService.updateWidget({ UserId: data.UserId, WidgetCode: this.widgetForm.get('Widget').value }).subscribe(response => {
        this.loading = false;
        this._messageService.sendSuccessMessageObject(this.validationMessage.add.success)
