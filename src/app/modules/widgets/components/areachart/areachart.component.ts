...
export class AreachartComponent implements OnInit {
  ...
  @Output() hideAreaChart: EventEmitter<any> = new EventEmitter<any>();
  ...
  deleteWidget() {
    ...
    if (confirmation) {
      ...
      this._dashboardService.deleteWidget({ UserId: data.UserId, WidgetCode: 'W003' }).subscribe(response => {
        this._messageService.sendSuccessMessageObject(response.Message);
        this.hideAreaChart.emit();
      })
    }
  }
}
